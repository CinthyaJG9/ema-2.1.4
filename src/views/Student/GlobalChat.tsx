...
const GlobalChat: FC = () => {
  ...
  const setMessage = ({ message, user }: Message) => {
  ...
  return (
    <main className='flex h-[90vh] grid-cols-1 bg-white dark:bg-[#22222A] lg:grid-cols-2'>
      <section className={` hidden transition-all md:block md:w-[25%] `}>
        <SearchChats />
        <div className='scrollbar-default scrollbar-thin scrollbar-track-transparent scrollbar-thumb-gray-300 scrollbar-track-rounded-full scrollbar-thumb-rounded-full dark:scrollbar-thumb-gray-700 h-3/4 w-full overflow-y-scroll py-5 md:pr-10 md:pl-6'>
          <User
            name='EMA'
            active={true}
            img='/img/logo3.png'
      ...
      <section className='flex h-full w-[75%] flex-col bg-gray-50 dark:bg-[#1E1F24]'>
        ...
        <div className='scrollbar-thin scrollbar-track-transparent scrollbar-thumb-gray-700 scrollbar-track-rounded-full scrollbar-thumb-rounded-full h-full overflow-y-scroll py-5 px-5 '>
          <Divider />
