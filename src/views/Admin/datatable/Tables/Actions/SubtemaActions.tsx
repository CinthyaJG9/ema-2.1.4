...
const SubtemaActions: FC<Props> = ({ id, nombre, valida, rowId, setRowId }) => {
  ...
  const handleSubmit = async (): Promise<void> => {
    setLoading(true);
    const result = await SubtemaService.delete({
      id_sub: id,
      nombre_sub: nombre,
      valida_sub: valida ? 1 : 0,
    });
