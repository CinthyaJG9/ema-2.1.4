...
const ParcialActions: FC<Props> = ({ id, nombre, valida, rowId, setRowId }) => {
  ...
  const handleSubmit = async (): Promise<void> => {
    setLoading(true);
    const result = await ParcialService.delete({
      id_par: id,
      nombre_par: nombre,
      valida_par: valida ? 1 : 0,
    });
