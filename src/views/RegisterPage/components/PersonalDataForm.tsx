...
import type { IPersonalData } from '@/interfaces'

import { useForm, Controller } from 'react-hook-form'
import { yupResolver } from '@hookform/resolvers/yup'
import { SchemaRegisterPersonal } from '@/utils'
...
const PersonalDataForm: FC = () => {
  ...
  const onSubmit = async (dataForm: IPersonalData) => {
    ...
    // console.log({ onSubmitData, dataForm })

    // clearTimeout(onSubmitData)
    setLoading(false)
  }
    ...
    >
      ...
      <div className='relative flex'>
        <button
          ...
          } items-center justify-between rounded-lg bg-secondary py-2 px-8 font-bold text-white shadow-lg transition-colors hover:bg-secondary-dark hover:shadow-secondary-dark focus:outline-none focus:ring-2 focus:ring-secondary-light focus:ring-opacity-50`}
        >
          {!loading ? (
            <>
              SIGUIENTE
